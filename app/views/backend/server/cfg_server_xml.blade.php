...
<Service>
 <Program>
  ...
  <Parameters>-server -name=arma3 "-profiles=C:\arma3server\instances\{{ $server->name }}\profile" "-pid=C:\arma3server\instances\{{ $server->name }}\server.pid" "-config=C:\arma3server\instances\{{ $server->name }}\server.cfg" "-cfg=C:\arma3server\instances\{{ $server->name }}\basic.cfg" "-par=C:\arma3server\instances\{{ $server->name }}\parameters.cfg" "-ranking=C:\arma3server\instances\{{ $server->name }}\logs\ranking.log" "-bepath=C:\arma3server\instances\{{ $server->name }}\battleye" -port={{ $server->port }}2</Parameters>
  <Delay>3000</Delay>
